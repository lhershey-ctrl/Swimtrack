...
**② Progress in Key Events — per-event season filter, redesigned 2026-08-01 (directly requested).** Each event card's season filter (below the event name, next to the chart) used to be single-select ("All" or exactly one season) and defaulted to "All." Now **multi-select** (clicking a season toggles it in/out of an `activeSeasons` Set, same interaction pattern as the event selector above it / `top5Active`) and **defaults to the last 2 seasons** (`availSeasons.slice(-2)`), not "All" — "All" is still one click away and selects every season at once. `renderCardData()` (inside `renderEvCards()`'s per-event closure) filters `evRes` by `activeSeasons.has(r.season)` instead of a single equality check. Chart/table structure (`makeEvChart`/`makeEvTable`) is completely unchanged — only which seasons feed them.
...
A per-coach roster view (`#tc-team`) with highlight cards (`_teamHighlights(roster)`, computed from `_teamSeasonRecap(D)` per swimmer): Most Improved, Top Points Swim, Busiest Competitor.

**"Needs Attention"** (`#teamAttentionSection`/`#teamAttention`, added 2026-07-31, redesigned 2026-08-01) is its own list section, not folded into the highlight cards — every swimmer who swims an event often this season (`>=TEAM_ATTENTION_MIN_SWIMS`, currently 5) with little-to-no improvement between their first and best time that season (`< TEAM_ATTENTION_MAX_PCT`, currently 2% — covers both zero and "marginal" improvement) gets a row (`_teamNeedsAttentionList(roster)`, sorted worst-first), not just a single roster-wide pick. **Thresholds were relaxed the day after shipping, from a real live example**: a swimmer with exactly 5 swims of an event and ~1.97% improvement fell just outside the original `>5`/`pct<=0` cutoffs on BOTH counts — `>5` (strict) excluded anyone at exactly 5 starts, and `pct<=0` excluded real-but-marginal improvement, which the user's own words ("not improving ... or improved marginally") clearly meant to include. `_teamSeasonRecap`'s `stagnantEv`/`stagnantPct`/`stagnantCount` fields (per-swimmer, their own single worst qualifying event) are unchanged in shape — `_teamNeedsAttentionList` just collects every swimmer with one instead of `_teamHighlights` picking only the single worst across the whole roster. An early-warning signal (possible stroke/technique or growth-related issue) that "most improved" alone can't surface. Mobile has no Team tab (see the 2026-07-19 nav-capacity note below).
...
- **61 scenarios** as of this writing, in `tests/scenarios/` (desktop + `mobile-*` twins covering the same behavior on each app where applicable).
