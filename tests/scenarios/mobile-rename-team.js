...
// negative case (a non-creator, non-owner coach sharing the same team never
// sees the rename field at all — the app owner CAN, see
// mobile-owner-bypass-rename-delete.js).
...
module.exports = async function run() {
  ...
  try {
    // ── Creator: sees + can use the "Current Team's Name" field ──
    {
      ...
      assert(/this team's name/i.test(bodyText), 'expected an editable "This Team\'s Name" field, got: ' + bodyText.slice(0, 800));
      steps.push({ desc: 'Team creator sees a "This Team\'s Name" field, pre-filled with the team\'s actual name', ok: true });
    ...
    // ── Non-creator, non-owner: a coach sharing the SAME team never sees the field ──
    {
      ...
      const bodyText = await page.evaluate(() => document.body.innerText);
      assert(!/this team's name/i.test(bodyText), 'a coach who did NOT create this team (and isn\'t the app owner) should never see a rename field for it, got: ' + bodyText.slice(0, 800));
